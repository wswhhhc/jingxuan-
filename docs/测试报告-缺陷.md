...
| **级别** | P2（一般） |
| **发现阶段** | 安全测试 |
| **问题描述** | 将脚本文件（`.sh`）或可执行文件（`.exe`）改名为 `.zip` 后，扩展名校验通过，且 Hutool 的魔数检测返回 `null`（无法识别），旧逻辑放行 `null` 导致绕过。攻击者可上传恶意脚本伪装成压缩包 |
| **修复方式** | `isRealTypeMismatch()` 中当 `realType` 为 `null` 且声明的扩展名在 Hutool 已知类型列表中时，判定为不匹配并拒绝。同时添加 `mov/mp4` 别名映射避免误判 |
